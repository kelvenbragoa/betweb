...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/typeoperations?page=${page}`,
      {
        params:{
...
axios.delete(`/typeoperations/${dataIdBeingDeleted}`)
.then(()=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Tipo de Operações</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos Tipo de Operações do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/admin/typeoperations/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        ...
                                                        <td>
                                                            <router-link :to="'/admin/typeoperations/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link>
                                                            <router-link :to="'/admin/typeoperations/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
