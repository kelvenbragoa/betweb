...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/company-transactions?page=${page}`,
      {
        params:{
...
axios.delete(`/company-transactions/${dataIdBeingDeleted}`)
.then(()=>{
...
<template>
    <div v-if="!loadingDiv">
        <h1 class="h3 mb-3">Transações</h1>
        
                        <div class="row">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Tabela dos Transações do sistema. {{ retriviedData.total }} registros encontrados.</h5>
                                        <h6 class="card-subtitle text-muted">Para procurar, digite na caixa de procura</h6>

                                        <router-link to="/company/transactions/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        <td>#{{ index + 1 }}</td>
                                                        <td>{{ actualData.company.name}}</td>
                                                        <td>{{ actualData.start_date}}</td>
                                                        <td>{{ actualData.end_date}}</td>
                                                        <td>{{ actualData.stevedors.length}}</td>
                                                        <td>
                                                            <span v-if="actualData.status == 0" class="badge bg-warning">
                                                                Aguardando Aprovação
                                                            </span>
                                                            <span v-if="actualData.status == 1" class="badge bg-success">
                                                                Em andamento
                                                            </span>
                                                            <span v-if="actualData.status == 2" class="badge bg-danger">
                                                                Expirou
                                                            </span>    
                                                        </td>
        
                                                        <td>
                                                            <!-- <router-link :to="'/company/transactions/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link> -->
                                                            <router-link :to="'/company/transactions/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
